...
function loadChatConfig() {
  if (!fs.existsSync(CHAT_CONFIG_PATH)) return {};
  try {
    const raw = fs.readFileSync(CHAT_CONFIG_PATH, "utf8");
    const parsed = JSON.parse(raw);
    return parsed && typeof parsed === "object" ? parsed : {};
  } catch (err) {
    console.error("Failed to read Chat.config:", err);
    return {};
  }
}
...
const STATIC_BLOCKLIST = new Set([
  `/${path.basename(DB_FILE)}`,
  `/${path.basename(DB_FILE)}-journal`,
]);
app.use((req, res, next) => {
...
const SUBPAGES_DIR = path.join(WEB_DIR, "subpages");
const SUBPAGE_NAMES = [
  "appeal",
  "appeals_list",
  "ban_info",
...
app.get(["/levels/search", "/levels/seach"], (req, res) => {
  res.sendFile(path.join(SUBPAGES_DIR, "levels", "search", "index.html"));
});
app.get("/levels/level", (req, res) => {
  res.sendFile(path.join(SUBPAGES_DIR, "levels", "level", "index.html"));
});
...
async function moderateTextWithOpenAI(text) {
  const key = process.env.OPENAI_API_KEY || CHAT_CONFIG.openaiApiKey;
...
function loadMailConfig() {
  ...
  const configFileSmtp =
    CHAT_CONFIG.smtp && typeof CHAT_CONFIG.smtp === "object"
      ? CHAT_CONFIG.smtp
      : null;
  ...
  if (configFileSmtp?.host && configFileSmtp.auth?.user && configFileSmtp.auth?.pass) {
    mailConfig = {
      host: configFileSmtp.host,
      port: Number(configFileSmtp.port) || 587,
      secure:
        typeof configFileSmtp.secure === "boolean"
          ? configFileSmtp.secure
          : (Number(configFileSmtp.port) || 587) === 465,
      auth: configFileSmtp.auth,
      from: configFileSmtp.from || configFileSmtp.auth.user,
    };
    return mailConfig;
  }
  mailConfig = null;
...
function sendTwoFactorCode(username, contact, code) {
  ...
  if (contact.email) {
    ...
    if (transporter) {
      ...
      console.warn("SMTP is not configured. Set SMTP_* env vars or add smtp to Chat.config.");
...
function sendPasswordResetEmail(username, email, link, expiresAt) {
  ...
  if (transporter) {
    ...
    console.warn(
      "SMTP is not configured. Set SMTP_* env vars or add smtp to Chat.config."
    );
